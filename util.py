...
def preprocess_attr_single(attr_names, attrA_list, attr_keys):
    '''
    Only take out the keys that is listed in attr_keys. Convert -1 to 0.
    '''
    attrA = []
...
def preprocess_image(dataA_list, image_size, phase='train'):
    imgA = ([get_image(img_path, image_size, phase=phase) 
        for img_path in dataA_list])
    imgA = np.array(imgA)
...
def preprocess_input(imgA, imgB, attrA, attrB, image_size, n_label):
    ...
    attrA = np.tile(np.reshape(attrA, [-1,1,1,n_label]),
        [1,image_size,image_size,1])
    attrB = np.tile(np.reshape(attrB, [-1,1,1,n_label]),
        [1,image_size,image_size,1])
...
def save_images(realA, realB, fake_B, image_size, sample_file, num=10):
    ...
    if np.array_equal(realA, realB): # for test
        ...
        img = np.concatenate((realA,realB,fake_B), axis=0)
        img = make3d(img, image_size, row=num, col=3)
    img = inverse_image(img)
    scm.imsave(sample_file, img)

def save_images_test(img_list, image_size, sample_file, num=10,col=4):
